...
BPB_Media	db	0xf8		; Legacy field, unused but should be 0xf8?
...
;;; FatSecNum BPB_FATSz * BPB_NumFats
;;; RootDirStartSec FatStartSec + FatSectors
;;; RootDirSectors 0
;;; DataStartSec RootDirStartSec + RootDirSectors
;;; DataSectors BPB_TotSec - DataStartSec
...
BS_BootCode32:
	...
	xor	bx,	bx
	mov	cl,	0x10
	mov	ax, RsvdSecCnt + SecPerFat * NumFATs
	call ReadSectors
	...
	;; first cluster is volume label, so skip first 32 byte
	;; second cluster is LFN, so skip temporary
	mov ax, [0x8200 + 32 + 32 + 26]		; cluster num at head of directory entry
	add ax, -2
	mov bx, SecPerClus
	mul bx
	add ax, RsvdSecCnt + SecPerFat * NumFATs
	mov	bx, 0x0820
	mov	es,	bx
	xor	bx,	bx
	mov	cl,	0x10
